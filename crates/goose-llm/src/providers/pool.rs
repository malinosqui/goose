...
impl ProviderPool {
    ...
    pub async fn get(&self) -> Result<PooledProviderGuard, ProviderError> {
...
#[derive(Debug)]
pub struct PooledProviderGuard<'a> {
    pool: &'a ProviderPool,
    provider: Option<Arc<dyn Provider>>,
    returned: bool,
}

impl<'a> Drop for PooledProviderGuard<'a> {
    fn drop(&mut self) {
...
#[async_trait]
impl<'a> Provider for PooledProviderGuard<'a> {
    async fn complete(
        &self,
        system: &str,
        messages: &[Message],
...
impl ProviderPoolManager {
    ...
    /// Get a provider from the appropriate pool
    pub async fn get_provider(
        ...
        let pool = self.get_or_create_pool(provider_name, provider_config.clone(), model_config.clone());
        pool.get().await
    }
}
...
/// Get a provider from the global pool
pub async fn get_pooled_provider(
    provider_name: &str,
    provider_config: Value,
    model_config: ModelConfig,
) -> Result<impl Provider, ProviderError> {
    global_pool_manager().get_provider(provider_name, provider_config, model_config).await
}